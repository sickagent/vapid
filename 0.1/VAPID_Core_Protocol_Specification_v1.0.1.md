...
| Message Kinds | **Обязательно** | Нормативный перечень типов протокольных сообщений |
| Message Schemas | **Обязательно** | Стандарты контрактов для `body.schema` и структуры `body.data` |
...
|-----------|--------|-------------|
| Consensus protocols | Исключено | Не относится к формату сообщений Core v1.0 |
| Reputation systems | Исключено | Не относится к wire format и правилам обработки |
| Payment / settlement | Исключено | Не относится к структуре протокольных объектов |
| Storage lifecycle | Исключено | Протокол описывает ссылку на артефакт, а не его жизненный цикл |
| Zero-knowledge proofs | Исключено | Не входят в обязательный набор Core v1.0 |
| System prompt governance | Исключено | Не является частью межагентного протокола сообщений |
| Blockchain anchoring | Исключено | Не является частью Core wire protocol |
...
Core v1.0 стандартизирует **пять** переносимых сущностей и нормативный набор типов сообщений и контрактов:
...
6. **Message Kinds** — перечисление допустимых типов сообщений
7. **Message Schemas** — идентификаторы и контракты полезной нагрузки
...
`kind` является частью протокольного контракта. Receiver **MUST** интерпретировать сообщение в соответствии со значением `kind`, а sender **MUST NOT** использовать нестандартизированные значения без явного namespaced имени.

#### Extended Kinds

Другие kinds **MUST** использовать namespaced naming как расширение протокольного пространства имён:
- `com.example.audit.request`
- `org.example.priority.update`
...
Для передачи стандартизированных метаданных исполнения ИИ:
...
- Receiver **MAY** использовать поле как часть протокольной обработки и аудита
...
`body.schema` является частью протокольного контракта. Он идентифицирует стандарт структуры `body.data`, а не требует обязательной сетевой загрузки схемы во время обработки сообщения.
...
Durable queues, strict ordering и иные транспортные гарантии не определяются Core v1.0.
...
- Контракт `body.data` **SHOULD** включать operation identifier
...
Artifact Descriptor используется для передачи ссылки на внешний артефакт и его проверяемых метаданных.
...
"critical_extensions": ["org.example.priority"]
```

Расширения изменяют только протокольные поля и semantics сообщения. Они не должны переопределять обязательные правила Core v1.0 для подписи, идентификаторов, временных полей и базовой валидации envelope.
...
- [x] Нормативный enum `kind` для core message types
- [x] Контрактная роль `body.schema` и структуры `body.data`
...
| 1.0.1 | 2026-03-25 | Уточнена область протокола как модель данных, типы сообщений и контрактные схемы; убраны platform- и roadmap-упоминания |
