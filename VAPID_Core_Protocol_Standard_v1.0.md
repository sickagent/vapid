...
Нумерация разделов сохранена в формате полной спецификации, поэтому документ начинается с разделов `5`, `6`, `7`, `11`, `14`, `18`, `19` и `20`.
...
- что такое `Arena` как контекст совместной работы агентов;
- какие роли агенты могут выполнять внутри арены;
...
`Arena` представляет собой общий контекст выполнения, в котором несколько агентов собираются для решения одной задачи или группы связанных задач. Внутри арены агенты обмениваются сообщениями, распределяют ответственность, используют общие правила доверия и работают в рамках общего бюджета ресурсов.

Арена задаёт:

- границы взаимодействия между агентами;
- контекст задачи, целей и ограничений;
- область применения квот через `quota_scope`;
- правила маршрутизации сообщений и интерпретации ролей.

### 5.2 Роли агентов в арене

Протокол не ограничивает систему тремя ролями, но базовая модель арены предполагает следующие типовые роли:

| Роль | Назначение |
| :--- | :--------- |
| `orchestrator` | Принимает задачу, декомпозирует её, назначает исполнителей, собирает промежуточные результаты и управляет потоком работы |
| `worker` | Выполняет выделенную подзадачу, производит артефакты, вызывает инструменты и возвращает результат в арену |
| `auditor` | Проверяет корректность, полноту, безопасность и соответствие результата правилам арены |

Агент **MAY** совмещать несколько ролей, если это допускается политикой арены, но при критических сценариях разделение ролей **SHOULD** использоваться для повышения надёжности и проверяемости.

### 5.3 Как агенты решают задачи внутри арены

Базовый жизненный цикл задачи в арене выглядит так:

1. `orchestrator` получает исходную задачу и определяет границы арены.
2. `orchestrator` разбивает задачу на подзадачи и распределяет их между `worker`-агентами.
3. `worker`-агенты обмениваются сообщениями, артефактами и промежуточными результатами в пределах правил арены.
4. `auditor` проверяет полученный результат или отдельные шаги исполнения.
5. `orchestrator` принимает финальное решение: завершить задачу, отправить на доработку или отклонить результат.

Если задача требует дополнительных ролей, например `planner`, `reviewer` или `tool-runner`, они рассматриваются как профили, расширяющие базовую модель арены.

### 5.4 Коммуникация и изоляция внутри арены

Сообщения между агентами внутри одной арены:

- **MUST** быть привязаны к контексту задачи и идентификатору арены;
- **MUST** проходить обычную валидацию `Receiver Pipeline`;
- **MUST** подчиняться тем же правилам подписи, отзыва ключей и квот ресурсов;
- **SHOULD** содержать достаточно метаданных, чтобы было видно роль отправителя и назначение сообщения.

Арена не отменяет изоляцию ресурсов: общий контекст сотрудничества не даёт агенту права выходить за пределы выданного `Capability Token` или использовать больше ресурсов, чем разрешено квотой.
...
- модель `Arena` как пространства совместной работы агентов;
- базовые роли `orchestrator`, `worker` и `auditor`;
...
| :------ | :----------------- | :----- |
| Arena | Общий контекст взаимодействия, ролей и ограничений для совместного решения задач | **MUST** |
